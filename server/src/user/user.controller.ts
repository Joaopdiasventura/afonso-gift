...
export class UserController {
  ...
  async create(
    ...
    if (existingUser) {
      ...
      if (existingUser.nickName == createUserDto.nickName)
        throw new BadRequestException("Já existe um usuário com esse apelido");
    }
    ...
    return this.encode((await this.userService.create(createUserDto)).email);
  }
  ...
  async login(@Body() loginUserDto: LoginUserDto) {
    ...
    if (!user)
      throw new BadRequestException("Não existe nenhum usuário com esse email");
  ...
  async findByEmail(@Param("email") email: string) {
    ...
    const followersCount = await this.userService.countFollowers(user.nickName);
    const followingCount = await this.userService.countFollowing(user.nickName);
  ...
  async remove(@Param("email") email: string) {
    ...
    if (user.picture) {
      const imagePath = join(__dirname, "..", "..", "uploads", user.picture);
